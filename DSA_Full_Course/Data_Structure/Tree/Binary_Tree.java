...
public class Binary_Tree {
    ...
    // Method for Travers Given tree by ZigZag Level Order
    public void zigZagOrderTraversal(Binary_Tree_Node root){
        if(root == null){
            return;
        }

        Deque<Binary_Tree_Node> lvl = new LinkedList<>();
        boolean flag = false;
        lvl.addFirst(root);
        lvl.addFirst(null);
        ...
        while (lvl.size() != 1) {
            if (!flag) {
                while (lvl.getLast() != null) {
                    Binary_Tree_Node last = lvl.removeLast();
